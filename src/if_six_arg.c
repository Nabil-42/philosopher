...
/*   Updated: 2024/07/23 16:43:59 by nabboud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	*philosopher_life_bis(void *params)
{
	...
	while (++i < pa->must_eat)
	{
		...
		if (check_died_bis(pa, index_p_s))
			return (NULL);
		(eating(pa, index_p_s), last_eat(pa, index_p_s));
		if (check_died_bis(pa, index_p_s))
			return (NULL);
		(give_back_fork(pa, index_p_s), sleeping(pa, index_p_s));
		if (check_died(pa, index_p_s))
			return (NULL);
		think(pa, index_p_s);
	}
	(pthread_mutex_lock(&pa->gate[4]), pa->philo_status[index_p_s].finish = 1);
